	...
	private static function parseAssignments(Tag $tag, bool $default): MacroTokens
	{
		...
		while ($tokens->nextToken()) {
			if ($var && !$hasType && $tokens->isCurrent($tokens::T_SYMBOL, '?', 'null', '\\')) { // type
				...
				throw new CompileException("Unexpected '{$tokens->currentValue()}' in {default $tag->args}", $tag->startLine);
		...
		if ($var === null) {
			...
			throw new CompileException("Unexpected end in {{$tag->name} {$tag->args}}", $tag->startLine);
		}
