...
final class TagParser extends TagParserData
{
	...
	public function parseFilters(): Node\ModifierNode // TODO: remove
	{
		trigger_error('Use parseModifier()');
			...
			do {
				if ($rule === 0) { // accept
					...
					if ($prevToken->is('echo', 'print', 'return', 'yield', 'throw', 'if', 'foreach', 'unset')) {
						throw new CompileException("Keyword '$prevToken->text' is forbidden in Latte", $prevToken->position);
					}
					...
					throw new CompileException('Unexpected ' . ($token->text ? "'$token->text'" : 'end'), $token->position);
				}
	...
	protected function parseOffset(string $str, Position $position): Scalar\StringNode|Scalar\IntegerNode
	{
